...
## Inventory capability owners before selecting tools

For each complex/foundational capability, inspect the project in this order before proposing implementation:

1. existing feature and shared components that already own the behavior;
2. installed dependencies, lockfile entries, workspace packages, generated clients, and their current adapters;
3. framework, language, browser, and platform facilities already available to the project;
4. internal primitives and compositions with relevant tests or stories;
5. credible mature libraries compatible with the actual runtime when the earlier layers leave a concrete gap.

Complex/foundational capabilities include graphs, editors, forms and validation, charts, overlays and focus management, drag and drop, virtualized tables or trees, scheduling, rich media, and other controls with difficult interaction, accessibility, data, or lifecycle behavior. Search by behavior and public API as well as package name. A custom-looking current surface may wrap a mature owner; a dependency in the manifest may be unused or deprecated. Confirm ownership through imports, configuration, call sites, and tests.

In a FULL workflow, reconcile this inventory with every v3 implementation-plan `capabilityRequirement` and its `capability`, `complexity`, and `constraints`. Do not treat an implementation-plan selection as current project evidence. If the inventory changes the candidate set or invalidates the selection, return the mismatch to the lifecycle owner before editing.
...
- Complex/foundational capability inventory:
- Implementation-plan requirement and decision IDs:
...
Create new only when candidates fail on semantics, ownership, accessibility, behavior, or maintainable extension. For a complex/foundational project-owned primitive, record at least two credible candidates, their evidence, the concrete gap, lifetime rationale, obligations, and validation before code. Absence of a candidate search is not evidence that the project must own the capability.
